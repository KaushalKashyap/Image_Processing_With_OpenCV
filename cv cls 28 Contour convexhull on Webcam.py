...
while True:
    ...
    mask1  = cv2.bitwise_not(mask) # it will simply reverse the output of mask.
    m_g = cv2.getTrackbarPos("Thresh", "Color Adjustments") #getting track bar value
    ret,thresh = cv2.threshold(mask1,m_g,255,cv2.THRESH_BINARY) # binary used here because we already converted the output using bitwise_not.
    dilata = cv2.dilate(thresh,(1,1),iterations = 6) # dilate will manage if any value comes distorted.
